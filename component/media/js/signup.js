...
function validatePassword()
{
	(function($) {
		...
		if (!akeebasubs_apply_validation)
		{
			...
			{
				$('#password').parent().parent().removeClass('error has-error success has-success');
				$('#password2').parent().parent().removeClass('error has-error success has-success');
				return;
			}
		}
		
		if(password == '') {
			$('#password').parent().parent().addClass('error has-error').removeClass('success has-success');
			$('#password2').parent().parent().removeClass('error has-error success has-success');
			$('#password_invalid').css('display','inline-block');
			akeebasubs_valid_form = false;
		} else {
			$('#password').parent().parent().removeClass('error has-error').addClass('success has-success');
			if(password2 != password) {
				$('#password2').parent().parent().addClass('error has-error').removeClass('success has-success');
				$('#password2_invalid').css('display','inline-block');
				akeebasubs_valid_form = false;
			} else {
				$('#password2').parent().parent().removeClass('error has-error').addClass('success has-success');
			}
		}		
	})(akeeba.jQuery);
}
...
function validateName()
{
	(function($) {
		...
		$('#name').parent().parent().removeClass('error has-error success has-success');
		if (!akeebasubs_apply_validation)
		{
			return;
		}
		...
		**/
		...
		if(invalidName) {
			$('#name').parent().parent().addClass('error has-error').removeClass('success has-success');
			$('#name_empty').css('display','inline-block');
			akeebasubs_valid_form = false;
			return;
		} else {
			$('#name').parent().parent().removeClass('error has-error').addClass('success has-success');
		}
	})(akeeba.jQuery);
}
...
function validateEmail()
{
	(function($) {
		...
		$('#email').parent().parent().removeClass('error has-error success has-success');
		$('#email2').parent().parent().removeClass('error has-error success has-success');
		var email = $('#email').val();
		...
		if(email == '') {
			$('#email').parent().parent().addClass('error has-error').removeClass('success has-success');
...
function validateAddress()
{
	...
	(function($) {
		...
		if(akeebasubs_personalinfo == 1)
		{
			if (!akeebasubs_apply_validation)
			{
				$('#address1').parent().parent().removeClass('error has-error success has-success');
				$('#country').parent().parent().removeClass('error has-error success has-success');
				$('#city').parent().parent().removeClass('error has-error success has-success');
				$('#state').parent().parent().removeClass('error has-error success has-success');
				$('#zip').parent().parent().removeClass('error has-error success has-success');
			...
			$('#address1').parent().parent().removeClass('error has-error success has-success');
			if(address == '') {
				$('#address1').parent().parent().addClass('error has-error');
				$('#address1_empty').css('display','inline-block');
				hasErrors = true;
			} else {
				$('#address1').parent().parent().addClass('success has-success');
				$('#address1_empty').css('display','none');
			}

			$('#country').parent().parent().removeClass('error has-error success has-success');
			if(country == '') {
				$('#country').parent().parent().addClass('error has-error');
				$('#country_empty').css('display','inline-block');
				hasErrors = true;
			} else {
				$('#country').parent().parent().addClass('success has-success');
				$('#country_empty').css('display','none');
			...
			*/
			...
			$('#city').parent().parent().removeClass('error has-error success has-success');
			if(city == '') {
				$('#city').parent().parent().addClass('error has-error');
				$('#city_empty').css('display','inline-block');
				hasErrors = true;
			} else {
				$('#city').parent().parent().addClass('success has-success');
				$('#city_empty').css('display','none');
			}

			$('#zip').parent().parent().removeClass('error has-error success has-success');
			if(zip == '') {
				$('#zip').parent().parent().addClass('error has-error');
				$('#zip_empty').css('display','inline-block');
				hasErrors = true;
			} else {
				$('#zip').parent().parent().addClass('success has-success');
				$('#zip_empty').css('display','none');
			}
		}
		else
...
function applyValidation(response, callback)
{
	...
	(function($) {
		...
		if (akeebasubs_apply_validation)
		{
			$('#username').parent().parent().removeClass('error has-error success has-success');
			if($('#username_valid')) {
				if(response.username) {
					$('#username').parent().parent().addClass('success has-success');
					...
					if($('#username').val() != '') {
						$('#username').parent().parent().addClass('error has-error');
			...
			$('#name').parent().parent().removeClass('error has-error success has-success');
			if(response.name) {
				$('#name').parent().parent().addClass('success has-success');
				$('#name_empty').css('display','none');
			} else {
				$('#name').parent().parent().addClass('error has-error');
				akeebasubs_valid_form = false;
				$('#name_empty').css('display','inline-block');
			}
			...
			$('#email').parent().parent().removeClass('error has-error success has-success');
			if(response.email) {
				$('#email').parent().parent().addClass('success has-success');
				$('#email_invalid').css('display','none');
			} else {
			...
			$('#email2').parent().parent().removeClass('error has-error success has-success');
			if(response.email2) {
				$('#email2').parent().parent().addClass('success has-success');
				$('#email2_invalid').css('display','none');
			} else {
				$('#email2').parent().parent().addClass('error has-error');
				akeebasubs_valid_form = false;
			...
			if(akeebasubs_personalinfo == -1)
			{
				...
				$('#state').parent().parent().removeClass('error has-error success has-success');
				if(response.state) {
					$('#state').parent().parent().addClass('success has-success');
					$('#state_empty').css('display','none');
				} else {
					$('#state').parent().parent().addClass('error has-error');
				...
				$('#businessname').parent().parent().removeClass('error has-error success has-success');
				if(response.businessname) {
					$('#businessname').parent().parent().addClass('success has-success');
					$('#businessname_empty').css('display','none');
				} else {
					$('#businessname').parent().parent().addClass('error has-error');
					if($('#isbusiness1').is(':checked')) {
						akeebasubs_valid_form = false;
					}
				...
				$('#occupation').parent().parent().removeClass('error has-error success has-success');
				if($('#occupation').val()) {
					$('#occupation').parent().parent().addClass('success has-success');
					$('#occupation_empty').css('display','none');
				} else {
					$('#occupation').parent().parent().addClass('error has-error');
					if($('#isbusiness1').is(':checked')) {
						akeebasubs_valid_form = false;
					}
		...
		if(akeebasubs_personalinfo == 1)
		{
			$('#vatnumber').parent().parent().parent().removeClass('warning has-warning success has-success');
				if(response.vatnumber) {
					$('#vatnumber').parent().parent().parent().addClass('success has-success');
					$('#vat-status-invalid').css('display','none');
					$('#vat-status-valid').css('display','inline-block');
				} else {
					$('#vatnumber').parent().parent().parent().addClass('warning has-warning');

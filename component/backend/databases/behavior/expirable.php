 ...
 * @author nicholas
 */
class ComAkeebasubsDatabaseBehaviorExpirable extends KDatabasebehaviorAbstract
{
	...
	public function _afterTableSelect(KCommandContext $context)
	{
		...
		foreach($rows as $row)
		{
			...
			if($row->publish_up && ($row->publish_up != '0000-00-00 00:00:00')) {
				...
				if( ($jNow->toUnix() < $jUp->toUnix()) && $row->enabled) {
					$row->enabled = 0;
					$triggered = true;
				}
			}
			...
			if($row->publish_down && ($row->publish_down != '0000-00-00 00:00:00')) {
				...
				if( ($jNow->toUnix() >= $jDown->toUnix()) && $row->enabled ) {
					$row->enabled = 0;
					$triggered = true;
				}
			}

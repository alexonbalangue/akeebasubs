...
class plgAkpaymentPaypalpaymentspro extends JPlugin
{
	private $ppName = 'paypalpaymentspro';
	private $ppKey = 'PLG_AKPAYMENT_PAYPALPAYMENTSPRO_TITLE';
	...
	public function __construct(&$subject, $config = array())
	{
		...
		$jlang->load('plg_akpayment_paypalpaymentspro', JPATH_ADMINISTRATOR, 'en-GB', true);
		$jlang->load('plg_akpayment_paypalpaymentspro', JPATH_ADMINISTRATOR, $jlang->getDefault(), true);
		$jlang->load('plg_akpayment_paypalpaymentspro', JPATH_ADMINISTRATOR, null, true);
	}
	...
	public function onAKPaymentGetIdentity()
	{
		...
		if(empty($ret['image'])) {
			$ret['image'] = rtrim(JURI::base(),'/').'/media/com_akeebasubs/images/frontend/PayPalPro1.gif';
		}
		return (object)$ret;
	}
	...
	private function logIPN($data, $isValid)
	{
		...
		$logFile = $logpath.'/akpayment_paypalpaymentspro_ipn.php';
		...
		if(!JFile::exists($logFile)) {
			...
			if(@filesize($logFile) > 1048756) {
				$altLog = $logpath.'/akpayment_paypalpaymentspro_ipn-1.php';

...
class plgAkpaymentPayFast extends plgAkpaymentAbstract
{
    ...
    public function onAKPaymentCallback($paymentmethod, $data)
    {
        ...
        $validationResult = $this->isValidIPN($data);

        // Added more info for debug
        $isValid = $validationResult['result'];
        $reason  = $validationResult['reason'];

        if(!$isValid) $data['akeebasubs_failure_reason'] = $reason;
    ...
    private function getCallbackURL()
    {
        ...
        if($sandbox) {
            ...
            return 'http://www.payfast.co.za/eng/query/validate';
        }
    }
    ...
    private function isValidIPN($data)
    {
        ...
        if(! in_array($_SERVER['REMOTE_ADDR'], $validIps)) {
            return array(
                'result' => false,
                'reason' => 'Remote host "'.$_SERVER['REMOTE_ADDR'].'" is not valid'
            );
        }
        ...
        if(md5($returnString) != $data['signature']) {
            return array(
                'result' => false,
                'reason' => 'Wrong signature match: '.md5($returnString).' vs '.$data['signature']
            );
        }
        ...
        if (!$fp) {
            ...
            return array(
                'result' => false,
                'reason' => 'Could not connect to PayFast server to check the transaction'
            );
        } else {
        ...
        return array(
            'result' => false,
            'reason' => 'Could not find the VALID token inside the return string: '.$remoteString
        );
    }
}
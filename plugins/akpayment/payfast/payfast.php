...
class plgAkpaymentPayFast extends plgAkpaymentAbstract
{
	...
	public function onAKPaymentCallback($paymentmethod, $data)
	{
		...
		try
		{
			...
			if (!$isValid)
			{
				$data['akeebasubs_failure_reason'] = 'Invalid response received.';
			}
		}
		catch (Exception $e)
		{
		...
		if ($isValid)
		{
			...
			if (!$isValid)
			{
				$data['akeebasubs_failure_reason'] = 'The custom_str1 is invalid';
			}
		}
	...
	private function isValidIPN($data)
	{
		...
		if (!in_array($thisIP, $validIps))
		{
			$ipList = implode(', ', $ips);
			throw new Exception("ITN comes from IP address $thisIP, does not belong in IP list $ipList", -1);
			return false;
		}
		...
		if ($md5 != $signature)
		{
			throw new Exception("Data signature validation failed, expected signature $md5, got $signature, validation string: $returnString", -1);
			return false;
		}

		// 3. Call PayFast server for validity check
		$fullHost = $this->getCallbackURL();
		$errno = 0;
		$errstr = '';

		$header = "POST /eng/query/validate HTTP/1.0\r\n";
		$header .= "Host: " . str_replace('ssl://', '', $fullHost) . "\r\n";
		$header .= "User-Agent: " . $_SERVER['HTTP_USER_AGENT'] . "\r\n";
		$header .= "Content-Type: application/x-www-form-urlencoded\r\n";
		$header .= "Content-Length: " . strlen($returnString) . "\r\n\r\n";

		$fp = fsockopen($fullHost, 443, $errno, $errstr, 10);

		if (!$fp)
		{
			// HTTP ERROR
			throw new Exception("HTTP Error $errno $errstr while cross-checking ITN, host $fullHost", $errno);
			return false;
		}
		else
		{
			...
			if (strcasecmp($res, "VALID") == 0)
			{
				return true;
			}
			else
			{
				throw new Exception("Error cross-checking ITN, expected VALID, got $res", -1);
			}
		}

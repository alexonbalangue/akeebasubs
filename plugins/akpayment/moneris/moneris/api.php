 ...
 * 
 * This file is provided by Moneris and is not covered by the GNU GPLv3 license
 */
...
class mpgGlobals{
	...
	public static function &getInstance()
	{
		...
		if(!is_object($instance)) {
			$instance = new mpgGlobals();
		}
		return $instance;
	}

	public function setSandbox($state = true)
	{
		if($state) {
...
class mpgHttpsPost{
	...
	function mpgHttpsPost($store_id,$api_token, $mpgRequestOBJ)
	{
		...
		curl_setopt($ch, CURLOPT_CAINFO, JPATH_ADMINISTRATOR.'/components/com_akeebasubs/assets/cacert.pem');
...
function toXML(){
	...
	{
		$dataValue = array_key_exists($txnTypeArray[$i], $txn) ? $txn[$txnTypeArray[$i]] : '';

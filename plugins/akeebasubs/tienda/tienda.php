...
class plgAkeebasubsTienda extends JPlugin
{
	...
	public function onAKUserRefresh($user_id)
	{
		...
		if(!empty($removeGroups)) {
			$protoSQL = 'DELETE FROM `#__tienda_usergroupxref` WHERE `user_id` = ' . $db->Quote($user_id) . ' AND `group_id` = ';
	...
	private function TiendaGroupToId($title)
	{
		static $groups = null;
		...
		if(is_null($groups)) {
			...
			$db = JFactory::getDBO();
			$sql = 'SELECT `group_name` AS `title`, `group_id` AS `id` FROM `#__tienda_groups`';
			$db->setQuery($sql);
	...
	private function parseGroups($rawData)
	{
		...
		foreach($lines as $line) {
			...
			foreach($groups as $groupTitle) {
				$groupIds[] = $this->TiendaGroupToId($groupTitle);
			}

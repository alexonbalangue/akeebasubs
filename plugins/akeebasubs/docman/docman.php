...
class plgAkeebasubsDocman extends JPlugin
{
	...
	public function onAKUserRefresh($user_id)
	{
		...
		foreach($addGroups as $gid) {
			$db->setQuery('SELECT * FROM `#__docman_groups` WHERE `groups_id` = '.$db->Quote($gid));
			...
			if($mustAdd) {
				...
				$sql = 'UPDATE `#__docman_groups` SET `groups_members` = '.$db->Quote($users).' WHERE `groups_id` = '.$db->Quote($gid);
				$db->setQuery($sql);
				$db->query();
			}
		}
	...
	private function DOCmanGroupToId($title)
	{
		static $groups = null;
		...
		if(is_null($groups)) {
			...
			$db = JFactory::getDBO();
			$sql = 'SELECT `groups_name` AS `title`, `groups_id` AS `id` FROM `#__docman_groups`';
			$db->setQuery($sql);
	...
	private function parseGroups($rawData)
	{
		...
		foreach($lines as $line) {
			...
			foreach($groups as $groupTitle) {
				$groupIds[] = $this->DOCmanGroupToId($groupTitle);
			}

...
class plgAkeebasubsJuga extends JPlugin
{
	...
	public function onAKUserRefresh($user_id)
	{
		...
		if(!empty($removeGroups)) {
			$protoSQL = 'DELETE FROM `#__juga_u2g` WHERE `user_id` = ' . $db->Quote($user_id) . ' AND `group_id` = ';
	...
	private function JUGAGroupToId($title)
	{
		static $groups = null;
		...
		if(is_null($groups)) {
			...
			$db = JFactory::getDBO();
			$sql = 'SELECT `title`, `id` FROM #__juga_groups';
			$db->setQuery($sql);
	...
	private function parseGroups($rawData)
	{
		...
		foreach($lines as $line) {
			...
			foreach($groups as $groupTitle) {
				$groupIds[] = $this->JUGAGroupToId($groupTitle);
			}
